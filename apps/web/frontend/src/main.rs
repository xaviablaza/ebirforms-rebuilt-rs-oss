...
fn js_error(value: JsValue) -> String {
    value.as_string().unwrap_or_else(|| "Request failed".into())
}
...
fn IntakeEditor(
    ...
    let (text, set_text) = create_signal(serde_json::to_string_pretty(&intake.payload).unwrap());
    let (revision, set_revision) = create_signal(intake.revision);
    let id = intake.id;
    let locked = intake.state != "draft";
    let persist = move |notify: bool| {
        let parsed = serde_json::from_str::<Value>(&text.get_untracked());
        spawn_local(async move {
            let body = match parsed {
                Ok(payload) => json!({"payload":payload,"revision":revision.get_untracked()}),
                Err(e) => {
                    set_message.set(e.to_string());
                    return;
                }
            };
            match request::<Value>(&format!("/intakes/{id}"), "PATCH", body).await {
                Ok(value) => {
                    if let Some(next) = value["revision"].as_i64() {
                        set_revision.set(next)
                    }
                    if notify {
                        set_message.set("Draft saved.".into())
                    }
                }
                Err(e) => set_message.set(e),
            }
    ...
    let submit = move |_| {
        spawn_local(async move {
            let payload = match serde_json::from_str::<Value>(&text.get_untracked()) {
                Ok(value) => value,
                Err(e) => {
                    set_message.set(e.to_string());
                    return;
            ...
            if let Err(e) = request::<Value>(
                &format!("/intakes/{id}"),
                "PATCH",
                json!({"payload":payload,"revision":revision.get_untracked()}),
            )
            .await
            {
                set_message.set(e);
            ...
            match request::<Value>(&format!("/intakes/{id}/submit"), "POST", json!({})).await {
                Ok(v) => {
                    ...
                    if let Ok(v) = request::<Vec<Intake>>("/intakes", "GET", Value::Null).await {
                        set_items.set(v)
                    }
                }
                Err(e) => set_message.set(e),
    ...
    view! {<div><p class="eyebrow">{format!("{} intake",intake.form_code)}</p><h2>{intake.reference.clone().unwrap_or_else(||"Draft return".into())}</h2>{if locked{view!{<div class="success"><h3>"Information received"</h3><p>"Our team will review and file this return. The official receipt will follow after filing."</p></div>}.into_view()}else{view!{<><p>"Complete the canonical form data below with your adviser. Changes autosave after a short pause; submitting locks the intake for review."</p><label>"Return data (JSON)"<textarea rows="24" prop:value=text on:input=autosave></textarea></label><div class="actions"><button class="secondary" on:click=save>"Save now"</button><button on:click=submit>"Send for review"</button></div></>}.into_view()}}</div>}
}
...
fn OperatorDetail(
    ...
    let delete = move |_| {
        spawn_local(async move {
            match request::<Value>(&format!("/operator/intakes/{id}"), "DELETE", json!({})).await {
                Ok(_) => {
                    set_selected.set(None);
                    if let Ok(v) =

...
mod tests {
    ...
    fn read_private_fixture(name: &str) -> Vec<u8> {
        let path = fixture_path(name);
        std::fs::read(&path).unwrap_or_else(|err| {
            panic!(
                "missing private fixture {path:?}: {err}. These files are intentionally gitignored; copy captured fixtures into fixtures/private/1601c/"
            )
        })
    }

    #[test]
    fn encrypt_matches_captured_1601c_v2_artifact() {
        let plaintext = read_private_fixture("plaintext-v2.xml");
        ...
        let expected = read_private_fixture("encrypted-v2.xml");
        assert_eq!(encrypted, expected);
    }

    #[test]
    fn decrypt_round_trips_captured_1601c_v2_artifact() {
        let encrypted = read_private_fixture("encrypted-v2.xml");
        ...
        let expected = read_private_fixture("plaintext-v2.xml");

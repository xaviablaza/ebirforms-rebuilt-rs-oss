...
mod tests {
    ...
    fn packages_public_1601c_fixture_with_expected_filename() {
        ...
        let expected_plaintext = fs::read(fixture_dir().join("official_plaintext.xml")).unwrap();
        let expected_filename =
            fs::read_to_string(fixture_dir().join("official_filename.txt")).unwrap();

        let package = build_submission_package("1601C", &input).expect("package fixture");

        let expected_payload = fs::read(fixture_dir().join("official_encrypted.xml")).unwrap();
